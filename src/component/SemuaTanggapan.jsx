...
import { sortSelect, FetchSemuaTanggapan } from "util/Fetches";
...
export default function SemuaTanggapan(props) {
  ...
  const { payload, lastFetch, orderBy } = state.semuaTanggapan.get();

  const optionChange = (option) => {
    FetchSemuaTanggapan({ action: "sortFetch", ext: option });
  };

  const fetchMore = () => {
    FetchSemuaTanggapan({ action: "moreFetch" });
  };
  ...
  const showDetails = (e) => {
    TriggerDetails({ id: e.id, status: e.status, action: "SemuaTanggapan" });
  };

  useEffect(() => {
    FetchSemuaTanggapan({ action: "effectFetch" });
  }, []);
