...
import { sortSelect, FetchLaporanPublik } from "util/Fetches";
...
export default function Laporanku(props) {
  ...
  const { payload, lastFetch, orderBy } = state.laporanPublik.get();

  const optionChange = (option) => {
    FetchLaporanPublik({ action: "sortFetch", ext: option });
  };

  const fetchMore = () => {
    FetchLaporanPublik({ action: "moreFetch" });
  };
  ...
  useEffect(() => {
    FetchLaporanPublik({ action: "effectFetch" });
  }, []);
